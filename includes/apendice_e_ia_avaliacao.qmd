...
O **LLMv02-Nq** foi utilizado em três turmas da disciplina **PDI-VC**, ministradas entre maio e agosto de 2026, totalizando aproximadamente uma centena de estudantes. O *feedback* por IA foi empregado de duas formas complementares:
...
Um aspecto importante do projeto diz respeito à privacidade dos dados. **Nome, e-mail, login, RA e CPF do estudante nunca são enviados à API do LLM**. Apenas o nome do arquivo contendo o código-fonte, o próprio código (com os comentários removidos) e o *prompt* da rubrica — que não contém dados pessoais — são transmitidos para processamento.
...
Quando a prova possui mais de uma questão por atividade VPL, os arquivos de código devem seguir o padrão `Q1.*`, `Q2.*` etc. — um arquivo por questão, com extensão livre conforme a linguagem escolhida pelo estudante —, e cada questão tem peso configurável individualmente em `grading.weights` no `config.yaml`. Quando a prova tem uma única questão, o sistema aceita qualquer nome de arquivo com extensão suportada, desde que seja o único arquivo de código presente na pasta de submissão.
...
#### Duas camadas adicionais de evidência {.unnumbered}

Para reduzir o risco de a LLM alucinar o tipo de questão ou a corretude do código — o risco central discutido na Seção D.7 —, o LLMv02-Nq passou a fornecer à IA duas fontes de evidência objetiva, extraídas automaticamente do próprio `execution.txt` gerado pelo VPL, além do código-fonte:
...
### D.5 Exemplo real (anonimizado): rubrica de uma prova com duas questões {.unnumbered}

O trecho a seguir ilustra, de forma ilustrativa, a saída gerada pelo LLMv02-Nq para um estudante de uma prova com **duas questões** (Q1 e Q2), usando o modelo `deepseek-chat`. O resumo comparativo aparece no topo do arquivo, seguido do código submetido e da avaliação por critério de cada questão. Neste caso, a Q1 foi identificada com confiança alta e nota próxima à do Moodle; a Q2 foi identificada com confiança baixa, por o código entregue divergir do que o enunciado oficial da questão pedia — ilustrando concretamente o mecanismo de sinalização descrito na Seção D.3.
...
│ Q2 (IA) : 26.7 / 50 pts                                      │
│ ──────────────────────────────────────────────────────────── │
│ Moodle : (Q1=47 + Q2=50) = 97 pts                            │
│ IA     : 76.7 / 100 pts                                      │
│ ──────────────────────────────────────────────────────────── │
│ Diferença (IA - Moodle): -20.3 pts                           │
│ ──────────────────────────────────────────────────────────── │
│ Q1: tipo identificado = C1 | confiança = Alta                │
│ Q2: tipo identificado = B  | confiança = Baixa (código não   │
│   corresponde ao enunciado da questão) ⚠️ REVISAR            │
└──────────────────────────────────────────────────────────────┘
```

**Q1 — avaliada com confiança alta:**

```
Tipo identificado: TIPO C1 (Erosão 2D — plana sem pesos)
Confiança: Alta

Critério 1 - 10.0/10.0 pts:
- Lê corretamente altura, largura, a matriz da imagem e as dimensões
  e valores do elemento estruturante B.
Critério 2 - 16.7/16.7 pts:
- Aplica mm.ero0(img, B) e o resultado bate com os testes automáticos
  do VPL em todos os casos fornecidos.
Critério 3 - 6.7/6.7 pts:
- Imprime a imagem resultante no mesmo formato da entrada.

NOTA FINAL: 33.3/33.3

Comentários para o aluno: Implementação correta e alinhada ao enunciado
sorteado (erosão plana sem pesos). Considere validar também casos de
borda extremos para reforçar a robustez do código.
```

**Q2 — avaliada com confiança baixa, por divergência entre enunciado e código:**

```
Tipo identificado: TIPO B (Dilatação 1D — com pesos)
Confiança: Baixa — o enunciado oficial da questão pede dilatação 1D
com pesos (clamp [0,255], B refletido), mas o código implementa apenas
a versão plana (sem uso dos valores de peso de B), sem clamp explícito.

Critério 1 - 10.0/10.0 pts:
- Lê corretamente N, M, o sinal e o vetor B.
Critério 2 - 10.0/16.7 pts:
- Implementa corretamente o deslocamento e a reflexão de B para a
  dilatação, mas ignora os valores de peso de B (trata-os como binários),
  não atendendo à variação "com pesos" pedida no enunciado. Não há
  clamp [0,255] no resultado.
Critério 3 - 6.7/6.7 pts:
- Imprime o sinal resultante corretamente formatado.

NOTA FINAL: 26.7/33.3

Comentários para o aluno: A lógica geral de dilatação 1D está correta,
mas a questão sorteada pedia especificamente a variação COM PESOS
(f[viz] + b[k], com clamp em [0,255]) e o código entregue implementa a
variação plana. Revise a leitura do enunciado da questão sorteada antes
de escolher a implementação.
```

Note que, nesse caso, a IA atribuiu **20,3 pontos a menos** do que a nota objetiva do VPL. A diferença por si só já seria um sinal para investigação (Seção D.7), mas aqui o próprio sistema já entrega ao professor a causa provável: a coluna `Revisar_Manualmente` do CSV é marcada "SIM" para esse estudante, com o motivo específico registrado na coluna de confiança da Q2 — reduzindo o tempo que o professor precisa gastar para descobrir, entre uma centena de estudantes, quais casos merecem atenção prioritária antes da atribuição da nota oficial.
...
### D.7 Riscos, limites éticos e responsabilidade docente {.unnumbered}
...
### D.8 Considerações finais {.unnumbered}
