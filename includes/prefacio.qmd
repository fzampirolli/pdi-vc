...
A obra é publicada em **dez combinações** — cada trilha de código (**Python**, 9 capítulos; **C++**, capítulos 1 a 5) em cinco idiomas (Português, Inglês, Francês, Espanhol e Italiano). Com o cache de tradução já povoado, uma regeneração completa (tradução, reexecução dos *notebooks*, renderização de HTML e PDF e publicação) leva cerca de **45 minutos** com quatro processos em paralelo; a **primeira** geração de um idioma novo, com o cache vazio, é bem mais lenta — cerca de **80 minutos** por combinação, como observado nas primeiras gerações de espanhol e italiano. A @tbl-00-tempos resume cada combinação (já com o cache povoado): número de páginas do PDF e tempo de renderização.
...
::: {.callout-note}
### Outras trilhas e idiomas {.unnumbered}
