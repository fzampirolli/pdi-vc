...
A obra é publicada em várias **combinações** — cada trilha de código (**Python** e **C++**) em cada idioma. Uma regeneração completa a partir do zero (tradução com cache, reexecução dos *notebooks*, renderização de HTML e PDF e publicação) leva cerca de **40 minutos** quando processada em paralelo com quatro processos simultâneos. A @tbl-00-tempos detalha o tempo de renderização de cada combinação.

| Combinação | Trilha | Idioma            | Tempo de renderização |
| :--------- | :----- | :---------------- | --------------------: |
...
: Tempo de renderização por combinação numa regeneração completa, com quatro processos em paralelo. A combinação-base em Python/Português apenas renderiza as saídas já registradas nos *notebooks*-fonte; as demais reexecutam todo o código. O tempo total de parede, incluindo a montagem do índice e a publicação, fica em torno de **38 minutos**. {#tbl-00-tempos}
...
Distinto do uso editorial descrito acima, o *pipeline* de geração multi-idioma (ver "Como este livro é produzido") emprega um modelo de linguagem como **componente de engenharia do sistema**: a tradução automática do conteúdo entre português e outros idiomas, com cache incremental que evita retraduzir trechos inalterados. Essa etapa usa a API paga do **DeepSeek** (modelo `deepseek-chat`); o livro completo já foi traduzido do português para o **inglês** e o **francês** (com o **espanhol** e o **italiano** em preparação), e os **Capítulos 1 a 5** contam ainda com uma trilha em **C++** que compila e executa de verdade — tudo a um custo acumulado abaixo de **US\$ 1**, graças ao cache incremental.

Algumas imagens do livro ainda contêm texto em português; elas serão futuramente traduzidas também para os demais idiomas, seguindo o mesmo padrão de sufixo utilizado nos demais arquivos gerados (por exemplo, `imagem_en.png`).
