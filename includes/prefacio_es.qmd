...
Hay algunos ajustes de fluidez, precisión y, principalmente, de consistencia en la descripción de la secuencia didáctica. También corregí el formato de *Safe Exam Browser*, *slides*, *feedback* y la referencia al MCTest. Sugiero esta versión:
...
La obra se publica en varias **combinaciones** — cada ruta de código (**Python** y **C++**) en cada idioma. Una regeneración completa desde cero (traducción con caché, reejecución de los *notebooks*, renderizado de HTML y PDF y publicación) tarda aproximadamente **40 minutos** cuando se procesa en paralelo con cuatro procesos simultáneos. @tbl-00-tempos detalla el tiempo de renderizado de cada combinación.

| Combinación | Ruta   | Idioma            | Tiempo de renderizado |
| :---------- | :----- | :---------------- | --------------------: |
| `py.pt`     | Python | Portugués (base)  | ~8 min                |
| `py.en`     | Python | Inglés             | ~32 min               |
| `py.fr`     | Python | Francés            | ~32 min               |
| `cpp.pt`    | C++    | Portugués          | ~28 min               |
| `cpp.en`    | C++    | Inglés             | ~32 min               |
| `cpp.fr`    | C++    | Francés            | ~22 min               |

: Tiempo de renderizado por combinación en una regeneración completa, con cuatro procesos en paralelo. La combinación base en Python/Portugués solo renderiza las salidas ya registradas en los *notebooks*-fuente; las demás reejecutan todo el código. El tiempo total de pared, incluido el montaje del índice y la publicación, ronda los **38 minutos**. {#tbl-00-tempos}
...
A diferencia del uso editorial descrito anteriormente, el *pipeline* de generación multilingüe (ver "Cómo se produce este libro") emplea un modelo de lenguaje como **componente de ingeniería del sistema**: la traducción automática del contenido entre portugués y otros idiomas, con caché incremental que evita retraducir fragmentos sin cambios. Esta etapa utiliza la API de pago de **DeepSeek** (modelo `deepseek-chat`); el libro completo ya ha sido traducido del portugués al **inglés** y al **francés** (con el **español** y el **italiano** en preparación), y los **Capítulos 1 a 5** cuentan además con una ruta en **C++** que compila y ejecuta de verdad — todo a un costo acumulado inferior a **US\$ 1**, gracias a la caché incremental.

Algunas imágenes del libro aún contienen texto en portugués; en el futuro también se traducirán a los demás idiomas, siguiendo el mismo patrón de sufijo utilizado en los demás archivos generados (por ejemplo, `imagem_en.png`).
