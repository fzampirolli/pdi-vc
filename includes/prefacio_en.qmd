...
Distinct from the editorial use described above, the multi-language generation *pipeline* (see "How this book is produced") employs a language model as a **system engineering component**: automatic translation of content between Portuguese and other languages, with an incremental cache that avoids retranslating unchanged passages. This step uses the **paid Anthropic API** (`claude-sonnet-5` model); the complete book has already been translated from Portuguese into **English** and **French**, and Chapter 1 additionally has a **C++** track that truly compiles and runs — all at a low cost, on the order of a few dollars, thanks to the incremental cache.
