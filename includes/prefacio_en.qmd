...
The work is published in **ten combinations** — each code track (**Python** and **C++**) in five languages (Portuguese, English, French, Spanish, and Italian). With the translation cache already populated, a full regeneration (translation, re-execution of the *notebooks*, HTML and PDF rendering, and publication) takes about **45 minutes** with four parallel workers; the **first** generation of a new language, with an empty cache, is much slower. @tbl-00-tempos summarizes each combination: PDF page count and rendering time.

| Combination | Track | Language | Pages | Rendering time |
| :--- | :--- | :--- | ---: | ---: |
| `py.pt` | Python | Portuguese (base) | 654 | ~8 min |
| `py.en` | Python | English | 644 | ~32 min |
| `py.fr` | Python | French | 666 | ~32 min |
| `py.es` | Python | Spanish¹ | 668 | ~80 min |
| `py.it` | Python | Italian¹ | 658 | ~80 min |
| `cpp.pt` | C++ | Portuguese | 434 | ~28 min |
| `cpp.en` | C++ | English | 426 | ~32 min |
| `cpp.fr` | C++ | French | 434 | ~22 min |
| `cpp.es` | C++ | Spanish | 430 | ~35 min |
| `cpp.it` | C++ | Italian | 426 | ~35 min |

: PDF page count and rendering time per combination (four parallel workers). The base Python/Portuguese combination only renders the outputs already stored in the source *notebooks*; the others re-execute all the code. ¹ First generation of the language, with an empty translation cache: every text segment is translated from scratch, which makes the initial generation much slower than subsequent regenerations. {#tbl-00-tempos}
