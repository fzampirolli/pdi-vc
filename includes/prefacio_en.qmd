...
The work is published in several **combinations** — each code track (**Python** and **C++**) in each language. A full regeneration from scratch (cached translation, re-execution of the *notebooks*, HTML and PDF rendering, and publication) takes about **40 minutes** when processed in parallel with four concurrent workers. @tbl-00-tempos breaks down the rendering time of each combination.

| Combination | Track  | Language          | Rendering time |
| :---------- | :----- | :---------------- | -------------: |
| `py.pt`     | Python | Portuguese (base) | ~8 min         |
| `py.en`     | Python | English           | ~32 min        |
| `py.fr`     | Python | French            | ~32 min        |
| `cpp.pt`    | C++    | Portuguese        | ~28 min        |
| `cpp.en`    | C++    | English           | ~32 min        |
| `cpp.fr`    | C++    | French            | ~22 min        |

: Rendering time per combination in a full regeneration, with four parallel workers. The base Python/Portuguese combination only renders the outputs already stored in the source *notebooks*; the others re-execute all the code. The total wall-clock time, including index assembly and publication, is around **38 minutes**. {#tbl-00-tempos}
...
Distinct from the editorial use described above, the multi-language generation *pipeline* (see "How this book is produced") employs a language model as a **system engineering component**: automatic translation of content between Portuguese and other languages, with an incremental cache that avoids retranslating unchanged passages. This step uses the paid **DeepSeek** API (`deepseek-chat` model); the complete book has already been translated from Portuguese into **English** and **French** (with **Spanish** and **Italian** in preparation), and **Chapters 1 to 5** additionally have a **C++** track that truly compiles and runs — all at a cumulative cost of under **US\$1**, thanks to the incremental cache.

Some images in the book still contain text in Portuguese; they will eventually be translated into the other languages as well, following the same suffix pattern used for the other generated files (for example, `image_en.png`).
