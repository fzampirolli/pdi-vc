...
No caso das duas provas descritas na @sec-contexto-desta-edicao, foi definido um conjunto de questões parametrizadas por prova, das quais um subconjunto é sorteado aleatoriamente para cada variação, totalizando **110 variações** distintas — uma por estudante matriculado nas turmas.
...
::: {.callout-important}
### Atenção {.unnumbered}
Cada acionamento do botão **Create-Variations** gera um novo conjunto de variações de exames. Após imprimir o PDF, é fundamental **não alterar os atributos da questão**, sob risco de invalidar a correção automática das provas já impressas ou aplicadas.
:::
...
O valor de `height` é sorteado uma única vez por variação (para aparecer no enunciado), enquanto `width` é sorteado a cada caso de teste, aumentando a robustez da correção automática. Note o uso do método `mm.drawImage()`, da própria `morph.py`, para formatar a matriz de saída como texto simples — facilitando a comparação exata entre a saída do aluno e o gabarito.
...
Na questão, o texto do enunciado não é fixo: ele é montado dentro do próprio bloco `[[def: ... ]]`, na variável Python `texto`, e inserido na descrição via `[[code:texto]]`. Isso permite, por exemplo, trocar frases, valores ou até a regra de ordenação em variações futuras da questão sem tocar no restante do código. A estrutura real utilizada (com trechos de sorteio de cenário omitidos por brevidade) é:
...
cases['description'] = [{"text": texto}]  # aparece no Moodle, diferente para cada aluno!
...
Note que `cases['description']` é o mesmo texto que alimenta tanto o PDF impresso quanto a atividade VPL no Moodle — como `texto` é gerado dentro do bloco `[[def: ...]]`, ele pode variar de aluno para aluno junto com os demais parâmetros sorteados, embora nesta questão específica apenas os dados numéricos (imagem, casos de teste) mudem entre variações.
